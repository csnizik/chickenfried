...
// Get site name from Azure (uppercase in logs, but use case-insensitive check)
$site_name = strtolower(getenv('WEBSITE_SITE_NAME'));

// Beta/Staging environment
if ($site_name === 'ars-va-beta-apps') {
  $trusted_hosts = [
    '^apps-stage\.ars\.usda\.gov$',
    '^ars-va-beta-apps\.ase-grin-staging-01\.appserviceenvironment\.us$',
  ];
}
// Production environment
elseif ($site_name === 'ars-va-prod-apps') {
  $trusted_hosts = [
    '^apps\.ars\.usda\.gov$',
    '^ars\-va\-prod\-apps\.ase\-grin\-prod\-01\.appserviceenvironment\.us$',
  ];
}
...
elseif (getenv('IS_DDEV_PROJECT') === 'true') {
  $trusted_hosts = [
    '^ars-apps-drupal\.ddev\.site$',
    '^127\.0\.0\.1$',
    '^localhost$',
  ];
}
else {
  // Fallback - allows container to start
  $trusted_hosts = [
...
$databases['default']['default'] = [
  ...
  'pdo' => [
    ...
    \PDO::MYSQL_ATTR_INIT_COMMAND => "SET SESSION net_read_timeout=600, SESSION net_write_timeout=600"
  ],
  'init_commands' => []
];
...
/**
 * OpenID Connect - Protected from config import
 */
if (getenv('AZURE_AD_CLIENT_ID')) {
  $config['openid_connect.client.windows_aad'] = [
    ...
    'settings' => [
      ...
      'authorization_endpoint' => getenv('AZURE_AD_AUTH_ENDPOINT'),
      'token_endpoint' => getenv('AZURE_AD_TOKEN_ENDPOINT'),
    ],
  ];

  $config['openid_connect.settings']['always_save_userinfo'] = TRUE;
  $config['openid_connect.settings']['connect_existing_users'] = TRUE;
  $config['openid_connect.settings']['override_registration_settings'] = FALSE;
  $config['openid_connect.settings']['end_session_enabled'] = TRUE;
  $config['openid_connect.settings']['user_login_display'] = 'replace';
  $config['openid_connect.settings']['redirect_login'] = 'user';
  $config['openid_connect.settings']['userinfo_mappings']['timezone'] = 'zoneinfo';
}
// if (getenv('AZURE_AD_CLIENT_ID')) {
//   $config['openid_connect.client.windows_aad'] = [
//     'id' => 'windows_aad',
...
/**
 * Azure App Service reverse proxy configuration.
 *
 * Azure terminates SSL at the load balancer and forwards requests over HTTP.
 * This tells Drupal to trust the X-Forwarded-* headers from Azure.
 */

if (!empty($site_name) && ($site_name === 'ars-va-beta-apps' || $site_name === 'ars-va-prod-apps')) {
...
/**
 * Temporary theme bypass: use only if no access to drush.
 * These settings override configuration stored in the database and should
 * remain commented out during normal operation. Only enable
 * them if the site cannot be accessed because the active theme or site
 * configuration is broken and you need to force a known-working theme
 * to recover access.
 * Why: enabling these bypasses Drupal's configuration system and the
 * values stored in the database, which can mask or prevent fixing the
 * underlying configuration problem. Use as an emergency recovery only.
 * How to enable temporarily:
 *   1) Uncomment the 3 lines below.
 *   2) Deploy.
 *   3) Use admin username/password to login.
 *   4) Set the site to maintenance mode and fix.
 *
 * UNCOMMENT ONLY TO TEMPORARILY RECOVER LOGIN
 * This should allow you to login and clear cache or disable components, theme, etc.
 */
