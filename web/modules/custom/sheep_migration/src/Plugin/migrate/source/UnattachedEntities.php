 ...
 *   group_plugin_id: sheep_entities_sheep_record
 *   exclude_group: 4
 * @endcode
 *
 * @MigrateSource(
...
class UnattachedEntities extends SourcePluginBase implements ContainerFactoryPluginInterface {
  ...
  protected function initializeIterator(): \Iterator {
    ...
    if (empty($entity_type)) {
      \Drupal::logger('sheep_migration')->error('UnattachedEntities: entity_type configuration is required.');
      return new \ArrayIterator([]);
    }

    // Validate entity type exists.
    if (!$this->entityTypeManager->hasDefinition($entity_type)) {
      \Drupal::logger('sheep_migration')->error('UnattachedEntities: entity_type @type does not exist.', ['@type' => $entity_type]);
      return new \ArrayIterator([]);
    }
